...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        guard let windowScene = (scene as? UIWindowScene) else {
            print("❌ ОШИБКА: Не удалось получить UIWindowScene")
            return
        }
        
        print("✅ SceneDelegate: willConnectTo вызван")
        ...
        if !(initialViewController is UINavigationController) {
            ...
            print("✅ SceneDelegate: View controller обёрнут в NavigationController")
        } else {
            window?.rootViewController = initialViewController
        }
        
        window?.makeKeyAndVisible()
        print("✅ SceneDelegate: Window создан и отображён")
    }
